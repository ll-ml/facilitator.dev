...
// PaymentRequiredResponse is what servers send with 402 status
...
// PaymentPayload is what clients send (base64 encoded) represents the decoded payment payload for a client's payment
...
type VerifyRequest struct {
	X402Version         int                 `json:"x402Version"`
	PaymentHeader       string              `json:"paymentHeader"` // Base64 encoded PaymentPayload
...
// Methods and functions on various types
...
func NewVerifyOK(payer string) VerifyResponse {
...
func NewVerifyFail(reason string) VerifyResponse {
	return VerifyResponse{
		IsValid:       false,
...
type Authorizer interface {
	Authorize(ctx context.Context, p *PaymentPayload, req *PaymentRequirements) VerifyResponse
}

type Settler interface {
	Settle(ctx context.Context, p *PaymentPayload, req *PaymentRequirements) (txHash string, err error)
}
